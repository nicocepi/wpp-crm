...
| 16 | `appointments.sql` | **Módulo de turnos**: tablas (settings, specialties, treatments, professionals, schedules, exceptions, appointments, appointment_audit, gcal_*), RLS por tenant, funciones `book_appointment` (anti-doble-reserva con advisory lock + cupo), `confirm_held_appointment`, `reopen_appointment` (re-valida cupo al volver a un estado que consume, ej. "Reabrir" completed/no_show → confirmed), `expire_appointment_holds`, trigger `updated_at`, realtime de `appointments`, unique de `gcal_connections.tenant_id` (una conexión por empresa). |
